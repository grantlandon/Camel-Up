...
def summarize_game_state_from_row(row):
    summary = {}
    for i in range(5):
        camel = f"c_{i}"
        summary[f"camel_{camel}_location"] = row[f"camel_{camel}_location"]
        summary[f"camel_{camel}_stack_location"] = row[f"camel_{camel}_stack_location"]
        summary[f"camel_{camel}_yet_to_move"] = row[f"camel_{camel}_yet_to_move"]
    return summary
    ...
    for game_id in tqdm(range(num_games), desc="Simulating Greedy Games"):
        ...
        for i in range(1, len(df)):
            ...
            if curr_round > prev_round:
                round_end_states[prev_round] = summarize_game_state_from_row(df.iloc[i])
        ...
        # Score round bets by checking the camel placements at the start of the next round
        for round_num, camel, player, idx, entry in round_bets:
            # Find the first row of the next round (this is the state immediately after the previous round ends)
            next_round_rows = df[df["actual_round"] == round_num + 1]
            if next_round_rows.empty:
                state_row = df.iloc[-1]  # fallback to final game state if next round doesn't exist
            else:
                state_row = next_round_rows.iloc[0]

            placements = get_camel_placements(state_row)
            if camel not in placements:
                print(f"[INVALID ROUND BET] Game {game_id} | Round {round_num} | Player {player} | Camel {camel} not in placements: {placements}")
                entry["payout"] = -1
                continue

            correct_place = placements.index(camel)
            if correct_place == 0:
                payout = [5, 3, 2]
            elif correct_place == 1:
                payout = [1, 1, 1]
            else:
                payout = [-1]
            placed_so_far = sum(
                1 for r, c, _, _, _ in round_bets[:idx] if r == round_num and c == camel
            )
            assigned_payout = payout[placed_so_far] if placed_so_far < len(payout) else payout[-1]
            entry["payout"] = assigned_payout
            if assigned_payout not in [5, 3, 2, 1, -1]:
                print(f"[UNEXPECTED ROUND PAYOUT] Game {game_id} | Round {round_num} | Player {player} | Camel {camel} | Place {correct_place} | Index {placed_so_far} | Payout {assigned_payout}")
        df_actions = pd.DataFrame(action_logs)
        df_actions.loc[df_actions["action_type"] == "move_camel", "payout"] = 1
        invalid_rolls = df_actions[(df_actions["action_type"] == "move_camel") & (df_actions["payout"] != 1)]
        if not invalid_rolls.empty:
            print("[INVALID ROLL PAYOUTS DETECTED]")
            print(invalid_rolls[["game_id", "turn_id", "player_id", "coin_delta", "payout"]])
        df_actions.loc[df_actions["action_type"] == "move_camel", "payout"] = 1

        # Score game bets with correct ordering using total round delta - known payouts
        df_actions = pd.DataFrame(action_logs)
        ...
        for (bet_type, camel), group in game_bet_df.groupby(["bet_type", "camel"]):
            if camel not in final_placements:
                print(f"[INVALID GAME BET] Game {game_id} | Player(s): {[r['player_id'] for _, r in group.iterrows()]} | Camel {camel} not in final placements: {final_placements}")
