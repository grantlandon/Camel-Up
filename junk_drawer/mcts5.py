...
class MCTSAgent:
    ...
    def select(self, node: MCTSNode, epsilon=0.1):
        """
        Select the child node to expand using the epsilon-greedy strategy.
        :param node: Current node.
        :param epsilon: Probability of selecting a random child for exploration.
        ...
        while node.state.active_game and node.children:
            ...
            if np.random.rand() < epsilon:
                # Exploration: Select a random child
                return np.random.choice(node.children)
            
            # Exploitation: Select the child with the highest value
            best_child = max(node.children, key=lambda child: child.value / (child.visits + 1e-6))  # Avoid division by zero
            node = best_child
